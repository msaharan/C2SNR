...
def constantfactor(redshift):
    return (9/4)* np.power(H0/c,4) * np.power(omegam0,2)*(fgrowth(redshift, 0.308, unnormed=False) * (1 + redshift))**2
#----------------------------------------------------------
...
for redshift in range(2,3):
    ...
    for L in range(l_plot_low_limit + 10, l_plot_upp_limit, err_stepsize):
        ...
        print("Without NL {}".format(np.sqrt(2 / ((2*L + 1) * delta_l * f_sky)) * angpowspec_without_j[L]))
...
plt.suptitle(r"Angular Power Spectrum (Gaussian)")
plt.legend()
plt.xscale("log")
plt.yscale("log")
#plt.ylim(1E-9,1E-7)
