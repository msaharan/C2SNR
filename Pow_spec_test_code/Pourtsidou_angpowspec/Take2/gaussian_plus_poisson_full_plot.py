...
fileout = open("gaussian_plus_poisson_full_plot_j_upp_limit_{}_lmax_{}.txt".format(j_upp_limit, l_max), "w")
...
for redshift in range(2,3):
    ...
    for L in range (0, int(l_max * 2**0.5)):
    ...
    for L in range(l_plot_low_limit + 10, l_plot_upp_limit,err_stepsize):
        ...
        delta_C_L[counter] = np.sqrt(2/((2*L + 1)*delta_l* f_sky)) *(angpowspec_without_j[L] + N_L[counter]) 
        #print("With NL {}".format(N_L[L]))
        plt.errorbar(l[L], angpowspec_without_j[L], yerr=delta_C_L[counter], capsize=3, ecolor='blue')
        fileout.write("{}   {}  {}\n".format(L,delta_C_L[counter], N_L[counter]))
        print("deltaC_L = {}\n".format(delta_C_L[counter]))
...
plt.ylim(1E-9,1E-7)
plt.xlim(10,1000)
plt.savefig("gaussian_plus_poisson_full_plot_j_upp_limit_{}_lmax_{}.pdf".format(j_upp_limit, l_max))
plt.show()
