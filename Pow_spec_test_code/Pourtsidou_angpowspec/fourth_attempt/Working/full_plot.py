...
def N4(ell, j):
    return integrate.nquad(N4_integrand, [(0, l_max), (0, l_max)], args = (ell, j), opts = [{'limit' : 5000}, {'limit' : 5000}])[0]

def N3_integrand(l1, l2, ell, j, redshift):
    small_l = int(np.sqrt(l1**2 + l2**2))
    return Cshot(redshift) * 2 * (C_l_array[ell, j] + C_l_N) + (C_l_array[abs(ell-small_l),j]  + C_l_N) / two_pi_squared
...
def noise_denominator_integrand(l1, l2, ell,j, redshift):
    small_l = int(np.sqrt(l1**2 + l2**2))                                                    
    return (C_l_array[ell, j]*ell*small_l + C_l_array[abs(ell-small_l), j] * ell * (ell - small_l) + ell**2 * Cshot(redshift)) / two_pi_squared
...
redshift = 2
D2_L = distance(redshift)**2 * (distance(2.085) - distance(1.915))
l_max = 10000
#l_max =  600
...
# Write data
###############################################################################
plot_err = open("./Text_files/plt_err_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_{}_temp.txt".format(j_upp_limit-1, l_max, eta), "w")
plot_curve = open("./Text_files/plt_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_{}_temp.txt".format(j_upp_limit-1, l_max, eta), "w")
plot_cshot = open("./Text_files/cshot_temp.txt", "w")
...
for L in tqdm(range(1, l_plot_upp_limit)):
    ...
    angpowspec_without_j_signal_in_final_plot[L] = angpowspec_without_j[L] * (L * (L + 1)) / (2 * 3.14)
...
plt.plot(x_2,y_2, label='Pourtsidou et al. 2014 (z=2)')
#------------------------------------------------------------------------------

###############################################################################
# Cshot
###############################################################################
L, Cshot  = np.loadtxt("./Text_files/cshot_temp.txt", unpack=True)

#plt.plot(L, (L * (L + 1) / (2 * 3.14)) *Cshot,color='black', label='Cshot')
plt.plot(L, Cshot,color='black', label='Cshot')
#------------------------------------------------------------------------------

L, CL = np.loadtxt('./Text_files/plt_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_0_temp.txt'.format(j_upp_limit - 1, l_max), unpack=True)
plt.plot(L, CL, label = 'This Work (z = 2)', color = 'blue')

L_err, CL_err, delta_CL, N_L = np.loadtxt('./Text_files/plt_err_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_0_temp.txt'.format(j_upp_limit - 1, l_max), unpack=True)
...
plt.savefig('./Plots/full_plot_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_0_temp_without_L_multipliers.pdf'.format(j_upp_limit - 1, l_max))
plt.show()
