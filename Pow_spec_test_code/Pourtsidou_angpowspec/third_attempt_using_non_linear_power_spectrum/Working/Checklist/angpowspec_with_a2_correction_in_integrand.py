...
###############################################################################
## To calculate the angular power spectrum using the non-linear matter power spectrum
###############################################################################
def angpowspec_integrand_without_j(z, ell, dist_s, constf):
    dist = distance(z)
    return constf * (1 - dist/dist_s)**2 * h**3 * PK.P(z, ell/dist) * (1 + z)**2/ hubble_ratio(z)
...
plt.legend()
plt.ylim(1E-10,1E-7)
plt.savefig("./Plots/angpowspec_with_a2_correction_in_integrand.pdf")
plt.savefig("./Plots/angpowspec_with_a2_correction_in_integrand.png")
plt.show()
