...
pars21 = model.CAMBparams(NonLinear = 1, WantTransfer = True, H0=67.3, omch2=0.1199, ombh2=0.02205, YHe = 0.24770, Do21cm = True, transfer_21cm_cl = True)
pars21.DarkEnergy.set_params(w=-1.13)
pars21.set_for_lmax(lmax=2500)
pars21.InitPower.set_params(ns=0.9603, As = 2.196e-09)
pars21.set_matter_power(redshifts=[0.], kmax=2.0)
results = camb.get_background(pars21)
k=np.exp(np.log(10)*np.linspace(-6,1,1000))
PK21 = get_matter_power_interpolator(pars21, nonlinear=1, kmax = 2)
...
# To store the data
###############################################################################
z_file_21 = open("./Text_files/non_linear_angpowspec_with_21.txt", 'w')
z_file = open("./Text_files/non_linear_angpowspec_without_21.txt", 'w')
#------------------------------------------------------------------------------
...
L , z_CL = np.loadtxt('./Text_files/non_linear_angpowspec_without_21.txt', unpack = True)
L , z_CL_21 = np.loadtxt('./Text_files/non_linear_angpowspec_with_21.txt', unpack = True)
