...
def z_angpowspec_integrand(z, ell, dist_s, constf):
    dist = a_distance(z)
    return 2 * 3.14 * constf * (1 - dist/dist_s)**2 * PK.P(z, ell/dist) * (1 + z)**2 / hubble_ratio(z)
...
def d_angpowspec_integrand(z, ell, dist_s, constf):
    dist = c_distance(z)
    return 2 * 3.14 * constf * (1 - dist/dist_s)**2 * PK.P(z, ell * (1 + z)/dist) * (1 + z)**2 
...
# To store the data
###############################################################################
z_file = open("./Text_files/z_valageas.txt", 'w')
d_file = open("./Text_files/d_valageas.txt", 'w')
#------------------------------------------------------------------------------
...
L , z_CL = np.loadtxt('./Text_files/z_valageas.txt', unpack = True)
L , d_CL = np.loadtxt('./Text_files/d_valageas.txt', unpack = True)
