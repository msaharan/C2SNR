...
def d_angpowspec_integrand(dist, ell, dist_s, constf):
    ...
    return 2 * 3.14 * constf * (1 - dist/dist_s)**2 * h**3 * (1 + z)**2 * PK.P(z, ell * (1 + z)/dist)
...
def z_angpowspec_integrand(z, ell, dist_s, constf):
    dist = a_distance(z)
    return 2 * 3.14 * constf * (1 - dist/dist_s)**2 * h**3 * (1 + z)**2 * PK.P(z, ell/dist)/ hubble_ratio(z)
...
PK = get_matter_power_interpolator(pars, nonlinear=True, kmax = 60)
#------------------------------------------------------------------------------
