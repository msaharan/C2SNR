...
import time
import /mnt/storage/pdata/Utility/Git/dyskun/C2SNR/experiment-parameters/p_2014_parameters as exp
import /mnt/storage/pdata/Utility/Git/dyskun/C2SNR/cosmological-parameters/p_2014_cosmology as cos 
...
C_lj_array = np.zeros((int(2**0.5 * l_ul), j_max))

### error bars in the final signal-to-noise ratio plot
delta_C_L = np.zeros(n_err_points)

### lensing reconstruction noise
#### noise array
N_l = np.zeros(n_err_points)
#### other terms in the noise expression
...
C_lj_data = open('./text-files/c_lj_data_z_{}_lmax_{}_jmax_{}.txt'.format(z_s, l_ul, j_max - 1), 'w')
