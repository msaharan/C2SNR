#To plot the noise (equation A20) from zaldarriaga_2006 paper
...
def integration(i,redshift):
    integ = integrate.dblquad(lambda x, x: np.power((xs[redshift] - x)/xs[redshift], 2)* np.interp(l[i]/x, kn,dkn),0,xs[redshift], 0, xs[redshift])[0]
    return integ
