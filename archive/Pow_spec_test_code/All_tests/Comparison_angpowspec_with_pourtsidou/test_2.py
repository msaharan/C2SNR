...
def d_angpowspec_integrand(dist, ell, dist_s, constf):
    ...
    return constf * (1 - dist/dist_s)**2 * PK.P(z, ell * (1 + z)/dist) / h**3
...
def d_angpowspec_integrand_nl(dist, ell, dist_s, constf):
    z = np.interp(dist, dist_red, red)
    return constf * (1 - dist/dist_s)**2 * PK_nl.P(z, ell * (1 + z)/dist) / h**3
...
def z_angpowspec_integrand(z, ell, dist_s, constf):
    dist = a_distance(z)
    return constf * (1 - dist/dist_s)**2 * PK.P(z, ell/dist) / (hubble_ratio(z) * h**3)
...
def z_angpowspec_integrand_nl(z, ell, dist_s, constf):
    dist = a_distance(z)
    return constf * (1 - dist/dist_s)**2 * PK_nl.P(z, ell/dist) / (hubble_ratio(z) * h**3)
...
# To store the data
###############################################################################
d_file = open("./Text_files/test_2_linear_distance.txt", 'w')
z_file = open("./Text_files/test_2_linear_redshift.txt", 'w')

d_file_nl = open("./Text_files/test_2_nonlinear_distance.txt", 'w')
z_file_nl = open("./Text_files/test_2_nonlinear_redshift.txt", 'w')
#------------------------------------------------------------------------------
...
L , d_CL = np.loadtxt('./Text_files/test_2_linear_distance.txt', unpack = True)
L , z_CL = np.loadtxt('./Text_files/test_2_linear_redshift.txt', unpack = True)

L_nl , d_CL_nl = np.loadtxt('./Text_files/test_2_nonlinear_distance.txt', unpack = True)
L_nl , z_CL_nl = np.loadtxt('./Text_files/test_2_nonlinear_redshift.txt', unpack = True)
