...
def angpowspec_integration_with_j(ell, j, redshift):
    constf = constantfactor(redshift)
    dist_s = distance(redshift)
    return integrate.quad(angpowspec_integrand_with_j, 0.0001, redshift, args = (ell, dist_s, constf, j))[0]
...
def N3_integrand(l1, l2, ell, j, redshift):
    small_l = int(np.sqrt(l1**2 + l2**2))
    return Cshot(redshift) * 2 * (angpowspec_with_j[ell, j] + (C_l)) + (angpowspec_with_j[abs(ell-small_l),j]  + (C_l)) / two_pi_squared
