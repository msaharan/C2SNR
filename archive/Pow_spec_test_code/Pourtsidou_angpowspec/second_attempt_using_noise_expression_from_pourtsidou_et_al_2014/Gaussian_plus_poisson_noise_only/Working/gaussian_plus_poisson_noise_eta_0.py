    ...
    for current_l_max in l_max_array:
        ...
        for j in range(j_low_limit, j_upp_limit):
            ...
            for L in range(l_plot_low_limit + 10, l_plot_upp_limit, err_stepsize):
                ...
                fileout.write("{}   {}\n".format(L_array[L_counter], N_L[j, L_counter, l_max_counter], eta))
                print("L = {}, N_L = {}\n".format(L, N_L[j, L_counter, l_max_counter, eta]))
                L_counter = L_counter + 1
            
            y_low_limit = N_L[j, 0, l_max_counter, eta] / 10
            y_upp_limit = N_L[j_low_limit, L_counter-1, 0, 0] * 10
            plt.ylim(y_low_limit , y_upp_limit)
            plt.scatter(L_array, N_L[j, : ,l_max_counter, eta], label = '({},{},$\eta_{}$)'.format(j, current_l_max, eta), s = 5)
            plt.legend()
