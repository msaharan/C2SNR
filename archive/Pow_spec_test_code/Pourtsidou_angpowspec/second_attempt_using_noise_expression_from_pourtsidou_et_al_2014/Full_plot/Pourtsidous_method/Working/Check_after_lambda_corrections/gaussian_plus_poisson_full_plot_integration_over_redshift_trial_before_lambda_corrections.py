...
def angpowspec_integrand_without_j(ell, z):
...
def angpowspec_integration_without_j(ell):
    return integrate.quad(lambda z: angpowspec_integrand_without_j(ell, z) , 0, redshift)[0]

# This function accounts for K_parallel also. 
# It uses the discretized version of K_parallel (its jth mode) (2Pi/box_length)*j.
def angpowspec_integrand_with_j(ell, z, j): 
    distance_at_redshift = np.interp(z, redshift_from_file, distance_from_file) 
...
def angpowspec_integration_with_j(ell, j): 
    return integrate.quad(lambda z: angpowspec_integrand_with_j(ell, z, j), 0, redshift)[0]
#------------------------------------------------------------------------------
...
def N3_integrand(ell, l1, l2,j):
    small_l = int(np.sqrt(l1**2 + l2**2))
    return (angpowspec_with_j[ell, j] + (C_l)) + (angpowspec_with_j[abs(ell-small_l),j]  + (C_l)) / two_pi_squared

def N3(ell, j):
    return Cshot(redshift) * 2 * integrate.dblquad(lambda l1,l2: N3_integrand(ell,l1,l2, j), 0, l_max, lambda l2: 0, lambda l2: l_max)[0]

def N2(ell, j):
    return Tz(redshift)**2 * 0.21**2 * integrate.dblquad(lambda l1,l2: N3_integrand(ell,l1,l2, j), 0, l_max, lambda l2: 0, lambda l2: l_max)[0] / (eta_D2_L**2 * 0.3**4)

def N1(ell, j):
    return Tz(redshift)**2 * j_max * 0.357 * 2 * (integrate.dblquad(lambda l1,l2: 1 , 0, l_max, lambda l2: 0, lambda l2: l_max)[0] / two_pi_squared) * integrate.dblquad(lambda l1,l2: N3_integrand(ell,l1,l2, j), 0, l_max, lambda l2: 0, lambda l2: l_max)[0] / (eta_D2_L**2 * 0.3**3)
...
###############################################################################
def noise_denominator_integrand(ell,j, l1, l2):
    small_l = int(np.sqrt(l1**2 + l2**2))
...
def noise_denominator_integration(ell,j):
    return integrate.dblquad(lambda l1,l2: noise_denominator_integrand(ell,j, l1, l2),0 , l_max, lambda l2: 0,lambda l2: l_max)[0] / two_pi_squared
...
fileout = open("./J_text_files/gaussian_plus_poisson_error_bar_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_{}_trial_before_lambda_corrections.txt".format(j_upp_limit-1, l_max, eta), "w")
...
for L in range(l_plot_low_limit + 8, l_plot_upp_limit, err_stepsize):
    ...
    for j in tqdm(range(j_low_limit, j_upp_limit)):
        ...
        N1_sum[L_counter] = N1_sum[L_counter] + N1(L, j)
        N2_sum[L_counter] = N2_sum[L_counter] + N2(L, j)
        N3_sum[L_counter] = N3_sum[L_counter] + N3(L, j)
...
plt.plot(L_array[l_plot_low_limit:l_plot_upp_limit], angpowspec_without_j_signal_in_final_plot[l_plot_low_limit:l_plot_upp_limit], color='blue', label='This work (z = {})'.format(redshift))
plt.xlabel('L')
...
plt.legend()
plt.ylim(1E-9,1E-7)
plt.savefig("./J_plots/gaussian_plus_poisson_full_plot_integration_over_redshift_j_upp_limit_{}_lmax_{}_eta_{}_trial_before_lambda_corrections.pdf".format(j_upp_limit - 1, l_max, eta))
plt.show()
#------------------------------------------------------------------------------
