...
### plot
plt.plot(k_l, Tz(z_s)**2 * PK_l.P(z_s, k_l), label = '$P_{21}(k)$')
#plt.plot(k_l, PK_l.P(z_s, k_l), label = '$P_\delta(k)$')
plt.xscale('log')
plt.yscale('log')
plt.xlabel('$k\;(1/Mpc)$', fontsize = fs)
plt.ylabel(r'$P_{21}(k)\;(mK^2\;Mpc^3$)', fontsize = fs)
plt.legend(fontsize = fs)
#plt.title('21-cm and matter power Spectrum (z = 2)')
plt.title('21-cm power Spectrum (z = 2)')
#plt.xlim(l_plot_min, l_plot_max)
#plt.ylim(4E-10, 3E-8)
plt.savefig('./plots/21cm-powspec_{}.pdf'.format(z_s), bbox_inches='tight')
#plt.savefig('./plots/21cm-matter-powspec_{}.pdf'.format(z_s), bbox_inches='tight')
plt.show()
