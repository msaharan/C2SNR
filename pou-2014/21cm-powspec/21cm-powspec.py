...
def noise_denominator_integrand(l1, l2, ell, j):
    ...
    return (C_lj_array[ell, j] * ell * l_var + C_lj_array[abs(ell-l_var), j] * ell * (ell - l_var))**2 / (L**2 * (2 * 3.14)**2 * 2 * ( C_lj_array[ell, j] + C_l_N)  * ( C_lj_array[abs(ell - l_var), j] + C_l_N))
...
N_l_data = open('./text-files/n_l_data_z_{}_lmax_{}_j_{}.txt'.format(z_s, l_ul, j_max), 'w')
...
### N_l
for L in range(l_plot_min + 8, l_plot_max, err_stepsize):
    j_sum = 0
    for j in tqdm(range(j_min, j_max)):
        d_square_l_sum = 0
        for l1 in range(1, l_ul, int(l_ul/10)):
            for l2 in range(1, l_ul, int(l_ul/10)):
...
### read data
#-- 'i' denotes 'input'
iL, iN_L = np.loadtxt('./text-files/n_l_data_z_{}_lmax_{}_j_{}.txt'.format(z_s, l_ul, j_max), unpack=True)

### plot N_l
#plt.plot(iL, iL * (iL + 1) * iN_L / (2 * 3.14), label = '$N_{L}$', linestyle = 'dashed') #-- N_l * l(l+1)/2pi
plt.plot(iL, iN_L, label = '$N_{L}$') #-- N_l only
