...
for j in tqdm(j_array):
    ...
    C_lj_data = open('./text-files/c_lj_data_z_{}_lmax_{}_j_{}.txt'.format(z_s, l_ul, j), 'w')
    ...
    ### read data
    #-- 'i' denotes 'input'
    iL, iC_L = np.loadtxt('./text-files/c_lj_data_z_{}_lmax_{}_j_{}.txt'.format(z_s, l_ul, j), unpack=True)

    ### plot
    plt.plot(iL, iL * (iL + 1) * iC_L / (2 * 3.14), label = 'j = {}'.format(j)) #-- C_lj * l(l+1)/2pi
...
plt.savefig('./plots/c_lj_l_l_plus_one_z_{}_lmax_{}_j_{}.pdf'.format(z_s, l_ul, j), bbox_inches='tight')
