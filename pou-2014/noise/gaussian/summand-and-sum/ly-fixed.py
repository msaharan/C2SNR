...
for j in tqdm(range(j_min, j_max)):
    d2l_integrand_data = open('./text-files/ly_{}_integrand_data_z_{}_lmax_{}_j_{}_L_{}.txt'.format(ly, z_s, lx_ul, j, large_L), 'w')
...
d2l_integrand_sum_data = open('./text-files/ly_{}_integrand_sum_data_z_{}_lmax_{}_jmax_{}_L_{}.txt'.format(ly, z_s, lx_ul, j_max, large_L), 'w')
...
iLn, id2l_integrand_sum = np.loadtxt('./text-files/ly_{}_integrand_sum_data_z_{}_lmax_{}_jmax_{}_L_{}.txt'.format(ly, z_s, lx_ul, j_max, large_L), unpack=True)
plt.plot(iLn, id2l_integrand_sum, label = 'F (j_max = {})'.format(j_max))

for j in tqdm(range(j_min, j_max)):
    iLn, id2l_integrand = np.loadtxt('./text-files/ly_{}_integrand_data_z_{}_lmax_{}_j_{}_L_{}.txt'.format(ly, z_s, lx_ul, j, large_L), unpack=True)
    plt.plot(iLn, id2l_integrand, label = '$f_j$; j = {}'.format(j))

plt.xscale('log')
plt.yscale('log')
plt.xlabel('$lx$', fontsize = fs)

plt.ylabel('$f_j$ and $F$', fontsize = fs)

plt.title('L = {}; ly = {}'.format(large_L, ly) + '; $z_{source} = $' + '{}'.format(z_s))
plt.legend(fontsize = fs)
#plt.xlim(lx_ll, lx_ul)

plt.savefig('./plots/ly_{}_integrand_sum_z_{}_lmax_{}_jmax_{}_L_{}.pdf'.format(ly, z_s, lx_ul, j_max, large_L), bbox_inches='tight')
plt.show()
