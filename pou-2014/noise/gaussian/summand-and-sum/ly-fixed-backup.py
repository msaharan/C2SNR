...
### angular power spectrum 
#-- signal
C_lj_array = np.zeros((2 * lx_ul, j_max))
#------------------------------------------------------------------------
...
for j in range(j_min, j_max):
    for l in range (lx_ll, int(1.5 * lx_ul)):
...
for j in tqdm(range(j_min, j_max)):
    d2l_integrand_data = open('./text-files/ly_{}_integrand_data_z_{}_lmax_{}_j_{}.txt'.format(ly, z_s, lx_ul, j), 'w')
    for l1 in range(lx_ll, lx_ul):
        ll = int((l1**2 + ly**2)**0.5)
        d2l_integrand_data.write('{}  {}\n'.format(ll, noise_denominator_integrand(ll, large_L, j)))
    d2l_integrand_data.close()


d2l_integrand_sum_data = open('./text-files/ly_{}_integrand_sum_data_z_{}_lmax_{}_jmax_{}.txt'.format(ly, z_s, lx_ul, j_max), 'w')
for l1 in tqdm(range(lx_ll, lx_ul)):
    ll = int((l1**2 + ly**2)**0.5)
    jsum = 0
    for j in range(j_min, j_max):
...
iLn, id2l_integrand_sum = np.loadtxt('./text-files/ly_{}_integrand_sum_data_z_{}_lmax_{}_jmax_{}.txt'.format(ly, z_s, lx_ul, j_max), unpack=True)
plt.plot(iLn, id2l_integrand_sum, label = 'Sum (j_max = {})'.format(j_max))

for j in tqdm(range(j_min, j_max)):
    iLn, id2l_integrand = np.loadtxt('./text-files/ly_{}_integrand_data_z_{}_lmax_{}_j_{}.txt'.format(ly, z_s, lx_ul, j), unpack=True)
...
plt.ylabel('d2l_integrand / d2l_integrand_sum', fontsize = fs)

plt.title('ly = {}'.format(ly) + '; $z_{source} = $' + '{}'.format(z_s))
plt.legend(fontsize = fs)
#plt.xlim(lx_ll, lx_ul)

plt.savefig('./plots/ly_{}_integrand_sum_z_{}_lmax_{}_jmax_{}.pdf'.format(ly, z_s, lx_ul, j_max), bbox_inches='tight')
plt.show()
