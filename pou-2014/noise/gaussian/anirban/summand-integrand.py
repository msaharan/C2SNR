...
for large_L in tqdm(range(l_plot_ll, l_plot_ul)):
    ...
    for j in tqdm(range(j_min, j_max)):
        noise_denominator_sum = noise_denominator_sum + noise_denominator(large_L, j)
    len_reco_noise_data.write('{}  {}\n'.format(large_L, 1/noise_denominator_sum))
...
for j in tqdm(range(j_min, j_max)):
    ...
    for l1 in range(l_ll, l_ul):
        d2l_integrand_data.write('{}  {}\n'.format(l1, noise_denominator_integrand(l1, phi_1, large_L, j)))
...
for l1 in tqdm(range(l_ll, l_ul)):
    jsum = 0
    for j in range(j_min, j_max):
        jsum = jsum + noise_denominator_integrand(l1, phi_1, large_L, j)
