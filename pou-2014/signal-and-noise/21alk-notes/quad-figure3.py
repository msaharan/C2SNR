...
import time
import zz_2006_parameters as exp
import zz_2006_cosmology as cos 
...
def noise_denominator(ell, j):
    return integrate.nquad(noise_denominator_integrand, [(0, l_max), (0, l_max)], args = (ell, j), opts = [{'limit' : 20000}, {'limit' : 20000}])[0]
...
## CAMB
########################################################################
pars = model.CAMBparams(NonLinear = 1, WantTransfer = True, H0 = 100 * h, omch2 = omegach2, ombh2 = omegabh2)
pars.DarkEnergy.set_params(w = -1)
pars.set_for_lmax(lmax = l_ul)
pars.InitPower.set_params(ns = ns)
#results = camb.get_background(pars)
results = camb.get_results(pars)
#print(results.get_sigma8()) #-- gives 0.9445
...
### plot
plt.plot(iLc, iC_L * iLc * (iLc + 1) / (2 * 3.14), label = 'This work')
