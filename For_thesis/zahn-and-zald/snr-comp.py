...
###############################################################################
    ...
    tz, k = np.loadtxt('/home/dyskun/Documents/Utility/Git/Msaharan/C2SNR/C2/data-files/tz_z{}_fine.txt'.format(int(red)), unpack = True)
"""
...
# Store parameters
###############################################################################
parout = open('./text-files/z_{}_parameters.txt'.format(redshift), 'w')
parout.write('T(z = 7.139) {}'.format(tee_z))
...
# Write data
###############################################################################
plot_err = open('./text-files/z_{}_plt_err_j_upp_limit_{}_lmax_{}.txt'.format(redshift, j_upp_limit-1, l_max), 'w')
plot_curve = open('./text-files/z_{}_plt_j_upp_limit_{}_lmax_{}.txt'.format(redshift, j_upp_limit-1, l_max), 'w')
...
"""
    ...
    L, CL = np.loadtxt('./text-files/z_{}_plt_j_upp_limit_{}_lmax_{}.txt'.format(redshift, j_upp_limit - 1, l_max), unpack=True)
    plt.plot(L, CL, label = 'Convergence APS (z = {})'.format(redshift), color = 'blue')

    L_err, CL_err, delta_CL, N_L = np.loadtxt('./text-files/z_{}_plt_err_j_upp_limit_{}_lmax_{}.txt'.format(redshift, j_upp_limit - 1, l_max), unpack=True)
...
plt.ylabel(r'$C_L$', size=16)
plt.legend()
plt.xlim(l_plot_low_limit, l_plot_upp_limit)
plt.savefig('./plots/snr-comp.pdf'.format(redshift, j_upp_limit - 1, l_max))
plt.show()
