...
def noise_denominator_integration(ell,j, redshift):
    ...
    return tee_z**2 * noise_denominator_integrand(0,0 , ell, j, redshift) * l_max**2
"""
...
def noise_denominator_integrand(l1, ell,j, redshift):
    small_l = int(l1)
    #print('Den Integrand {}'.format((C_l_array[ell, j]*ell*small_l + C_l_array[abs(ell-small_l), j] * ell * (ell - small_l) + ell**2 * Cshot) / (two_pi_squared * tee_z**2)))
    return (C_l_array[ell, j]*ell*small_l + C_l_array[abs(ell-small_l), j] * ell * (ell - small_l) + ell**2 * Cshot) / (two_pi_squared * tee_z**2)

def noise_denominator_integration(ell,j, redshift):
    return tee_z**2 * integrate.quad(noise_denominator_integrand, 0, l_max, args=(ell, j,  redshift))[0]
    #return tee_z**2 * integrate.nquad(noise_denominator_integrand, [(ell, l_max), (ell, l_max)] , args = (ell, j, redshift), opts = [{'limit' : 50, 'epsabs' : 1.49e-11, 'epsrel' : 1.49e-11}, {'limit' : 50, 'epsabs' : 1.49e-11, 'epsrel' : 1.49e-11}])[0] / 100000
"""
#------------------------------------------------------------------------------
...
C_l_N = pc2n_s2(pc2, k, redshift) / D2_L
...
parout.write('P_CII_N {}'.format( pc2n_s2(pc2, k, redshift) ))
parout.write('C_l_N {}'.format(C_l_N))
parout.close()

#print('Cshot {}\n'.format(Cshot))
#print('T(z = 6.349) {}\n'.format(tee_z))
#print('P_CII_N {}\n'.format( pc2n_s2(pc2, k, redshift) ))
#print('C_l_N {}\n'.format(C_l_N))
#------------------------------------------------------------------------------
"""
...
l_fil, j_fil, clj = np.loadtxt('./text-files/clfile_j_{}_l_{}_z_{}.txt'.format(j_plot_upp_limit, l_max, redshift), unpack=True)
