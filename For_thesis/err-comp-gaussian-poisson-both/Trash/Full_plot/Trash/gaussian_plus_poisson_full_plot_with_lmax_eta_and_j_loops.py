...
delta_C_L = np.zeros((j_max, n_err_points, len(l_max_array), len(eta_array)))
...
for eta in eta_array:
    ...
    for current_l_max in l_max_array:
        ...
        for j in range(j_low_limit, j_upp_limit):
            print('-------------j = {}----------------'.format(j))
            fileout = open("./J_text_files/gaussian_plus_poisson_error_bar_integration_over_distance_j_upp_limit_{}_lmax_{}_eta_{}.txt".format(j,current_l_max, eta), "w")
            ...
            for L in range(l_plot_low_limit + 10, l_plot_upp_limit, err_stepsize):
                ...
                N_L[j, L_counter, l_max_counter, eta] = (L**2 * (N0_sum[L_counter] + N1_sum[L_counter] + N2_sum[L_counter] + N3_sum[L_counter] + N4(L, redshift)) ) / (noise_denominator_sum[L_counter]**2)
                delta_C_L[j, L_counter, l_max_counter, eta] = np.sqrt(2/((2*L + 1)*delta_l* f_sky)) *(angpowspec_without_j[L] + N_L[j, L_counter, l_max_counter, eta]) 
                angpowspec_without_j_error_bar[L_counter] = angpowspec_without_j[L]
                fileout.write("{}   {}   {}\n".format(L, delta_C_L[j, L_counter, l_max_counter, eta], N_L[j, L_counter, l_max_counter, eta]))
        ...
        L_counter = 0
        for L in range(l_plot_low_limit + 10, l_plot_upp_limit, err_stepsize):
            plt.errorbar(L, angpowspec_without_j[L], yerr=delta_C_L[j_upp_limit - 1, L_counter, l_max_counter, eta], capsize=3, ecolor='blue')
            L_counter = L_counter + 1
        plt.plot(L_array, angpowspec_without_j[l_plot_low_limit:l_plot_upp_limit], color='blue', label='This work (z = {})'.format(redshift))
        plt.legend()
        plt.ylim(1E-9,1E-7)
        plt.savefig("./J_plots/gaussian_plus_poisson_full_plot_integration_over_distance_j_upp_limit_{}_lmax_{}_eta_{}.pdf".format(j,current_l_max, eta))
...
"""
y_low_limit = delta_C_L[j, 0, l_max_counter, eta] / 10
y_upp_limit = delta_C_L[j_low_limit, L_counter-1, 0, 0] * 10
plt.ylim(y_low_limit , y_upp_limit)
plt.legend()
plt.savefig("./J_plots/gaussian_plus_poisson_error_bar_integration_over_distance_j_upp_limit_{}_lmax_{}_eta_{}.pdf".format(j,current_l_max, eta))
"""
