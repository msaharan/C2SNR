...
pars = model.CAMBparams(NonLinear = 0, WantTransfer = True, H0 = 100 * h, omch2=omegach2, ombh2=omegabh2) #-- H0 in kms^-1/Mpc as per the documentation
...
#PK = get_matter_power_interpolator(pars, nonlinear=True, kmax = k_max(l_max, z_s), k_hunit = True, hubble_units = True)
...
for jn in j_array:
    ...
    #aps_l_data = open('./text-files/angpowspec-l-lmax-{}-h-true.txt'.format(l_max), 'w')
    aps_lj_data = open('./text-files/angpowspec-lj-lmax-{}-j-{}-z-{}-h-true.txt'.format(l_max, jn, z_s), 'w')
...
## read
#L, CL = np.loadtxt('./text-files/angpowspec-l-lmax-{}-h-true.txt'.format(l_max), unpack=True)

for jn in j_array:
    L, C_lj = np.loadtxt('./text-files/angpowspec-lj-lmax-{}-j-{}-z-{}-h-true.txt'.format(l_max, jn, z_s), unpack=True)
...
plt.title("Mpc/h units")
#plt.ylim(4E-10, 3E-8)
#plt.savefig('./plots/angpowspec-lmax-{}-h-true.pdf'.format(l_max), bbox_inches='tight')
plt.savefig('./plots/angpowspec-lj-lmax-{}-z-{}-h-true.pdf'.format(l_max, z_s), bbox_inches='tight')
plt.show()
