...
def noise_denominator_integrand(l1, l2, ell,j, redshift):
    ...
    #time.sleep(0.05)
    #igd.write('{}   {}  {}  {}  {}   {}'.format(l1, l2, small_l, ell, j, redshift, (C_l_array[ell, j]*ell*small_l + C_l_array[abs(ell-small_l), j] * ell * abs(ell - small_l) + ell**2 * Cshot) / (two_pi_squared * tee_z**2)))
    ...
    for j in range(j_low_limit, j_upp_limit):
        for l1 in range(0, l_max):
            for l2 in range(0, l_max):
                res_sml = res + noise_denominator_integrand(l1, l2, ell, j, redshift)
        res_j = res_j + res_sml
    res_L = res_L + res_j
    igd.write('L {} N_den {}'.format(L, res_L**2))
"""
for L in tqdm(range(l_plot_low_limit, l_plot_upp_limit, err_stepsize)):
    res_j = 0
    for j in tqdm(range(j_low_limit, j_upp_limit)):
        res_sml = 0
        for l1 in range(0, l_max):
