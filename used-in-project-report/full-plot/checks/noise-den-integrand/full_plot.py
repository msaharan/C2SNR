...
ell = 500
n = 0
for L in tqdm(range(l_min, int(l_max * 2**0.5))):
    jn=0
    for j in range(j_low_limit, j_upp_limit):
        C_l_array[n, jn] = C_lj(L, j, redshift)
        jn = jn+1
    n = n+1

ca = np.zeros(l_max - l_min)
#la =  np.zeros((l_max - l_min, j_upp_limit - j_low_limit))
la =  np.zeros(l_max - l_min)
n = 0
for small_l in tqdm(range(l_min, l_max)):
    la[n] = small_l
    jn = 0
    ca_sum = 0
    for j in tqdm(range(j_low_limit, j_upp_limit)):
        ca_sum = ca_sum + (C_l_array[n, jn]*ell*small_l + C_l_array[abs(ell-small_l), jn] * ell * (ell - small_l) + ell**2 * Cshot) / (two_pi_squared)
        #ca_sum = ca_sum + (C_l_array[n, jn]*ell*small_l + C_l_array[abs(ell-small_l), jn] * ell * (ell - small_l) + ell**2 * Cshot) / (two_pi_squared * tee_z**2)
        jn = jn + 1
    ca[n] = ca_sum
    n = n + 1
